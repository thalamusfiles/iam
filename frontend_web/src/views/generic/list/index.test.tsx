...
describe('GenericList', () => {
    ...
    it('Exibi modal com filtros e troca os filtros?', async () => {
        ...
        await ctrl!.search();
        expect(ctrl!.filtersDefs).toHaveLength(customListDef.filters.length);
        ...
        ctrl!.applyFilters();
        expect(ctrl!.filtersApplied).toHaveLength(customListDef.filters.length);
...
const tableProtocolHead: TableHead = { colname: 'protocol', title: 'protocol', type: AttributeType.Text, order: order++, sortable: true, show: false, }
const tableCreatedAtHead: TableHead = { colname: 'createAt', title: 'Created At', type: AttributeType.DateTime, order: order++, sortable: true, show: false, }
...
class TestGraphQLDatasourceMock implements GraphQLInterface {
     ...
     */
    listUserDefs(): Promise<ListUserDefs> {
        const listUserDefs: ListUserDefs = {
            defaultList: ListDefinition,
            customLists: [],
        };
        return Promise.resolve(listUserDefs);
    };
 ...
 * Listagem Generica
 */
class TestGenericList extends GenericList {
    constructor(props: any) {
        super({
            ctrl: ctrlInstance('testGeneric', new TestGraphQLDatasourceMock()),
            ...props
        })
    }
}