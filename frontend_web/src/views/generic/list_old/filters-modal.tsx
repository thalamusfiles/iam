...
@inject('ctrl')
@observer
export class FiltersModal extends React.Component<{ ctrl?: CommonListStore; __?: Function }> {
  render() {
...
@inject('ctrl')
@observer
export class FiltersResume extends React.Component<{ ctrl?: CommonListStore; __?: Function }> {
  render() {
    const { __, ctrl } = this.props;
...
@inject('ctrl')
@observer
export class FastFilters extends React.Component<{ ctrl?: CommonListStore; __?: Function; inSide?: boolean }> {
  render() {
    const { __, ctrl } = this.props;
    ...
    const span = this.props.inSide ? 12 : 4 + hasMultiple;
...
function CascadeFilterGroup({ ctrl, filters }: { ctrl: CommonListStore; filters: FilterDef[] }) {
...
function FilterButton({ ctrl, filter }: { ctrl: CommonListStore; filter: FilterDef }) {
  return (
